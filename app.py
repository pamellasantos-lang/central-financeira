...
# --- ESTILIZAÇÃO CSS EXECUTIVA E RESPIRO ENTRE QUADROS ---
st.markdown("""
<style>
    /* Respiro superior e margens da página */
    .block-container {
    ...
    /* Customização dos Containers Nativos com Margem Inferior Reforçada */
    ...
    /* Botões em formato de quadradinho para escolha do mês */
...
# --- FUNÇÕES DE LIMPEZA E TRATAMENTO DE DADOS (DEFINIDAS NO TOPO) ---
...
# --- HEADER SUPERIOR EM CARD ENVELOPADO ---
...
with st.container(border=True):
    ...
    with col_rec_box:
        ...
        <div style="background:#F8FAFC; padding:16px; border-radius:8px; border:1px solid #CBD5E1;">
            ...
            <hr style="margin:6px 0; border:0.5px solid #CBD5E1;">
            <div style="font-size:0.85rem; font-weight:bold; color:#0F172A; margin-bottom:2px;">💰 Total Operacional em Conta</div>
...
# --- 5. MAPEAMENTO GERAL DE DÍVIDAS (TEXTUAL EM 2 BLOCOS) ---
with st.container(border=True):
    ...
    def calcular_progresso_divida(df_base, tipo_lista="atrasadas"):
        ...
        for idx, row in df_base.iterrows():
            ...
            # Cruzamento com Saídas apenas se for do tipo 'Parcelamento', 'Acordo' ou 'Dívida'
            if not df_saidas.empty and not mask_parcelamentos.empty and mask_parcelamentos.any():
                ...
                # Busca por fragmentos do nome do credor
                partes_credor = [p for p in credor.split() if len(p) > 3]
                query_search = '|'.join(partes_credor) if partes_credor else credor
                
                mask_match = df_saidas_parc[col_desc_sai].astype(str).str.contains(query_search, case=False, na=False)
    ...
    with col_div1:
        ...
        if div_atrasadas:
            ...
            st.info("Nenhuma dívida atrasada localizada.")

    # BLOCO 2: DÍVIDAS NO PRAZO
    with col_div2:
