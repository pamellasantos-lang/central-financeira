...
st.markdown("""
<style>
    /* Redução drástica da margem no topo da página */
    .block-container {
    ...
    /* Customização dos Containers Nativos */
    ...
    /* Botões para seleção de Mês (Remove as bolinhas) */
...
def obter_coluna_valor(df):
    ...
    cols = [c for c in df.columns if any(p in c.lower() for p in ['valor', 'total', 'receber', 'saldo', 'devedor', 'parcela']) and 'juros' not in c.lower()]
...
with st.container(border=True):
    col_titulo, col_filtros = st.columns([1.5, 2.5])
    with col_titulo:
...
if not df_saidas.empty:
    try:
        ...
        # Máscara rigorosa para identificar parcelamentos
...
# --- 3. CAIXINHA DE ESSENCIAIS MENSAIS (Sem Distribuição) ---
...
# --- 4. RECEITA OPERACIONAL EM CONTA E JANELAS ---
with st.container(border=True):
    ...
    with col_rec_box:
        st.markdown(f"""
        <div style="background:#F8FAFC; padding:18px; border-radius:8px; border:1px solid #CBD5E1;">
            <div style="font-size:0.9rem; font-weight:bold; color:#0F172A; margin-bottom:2px;">📅 Janela Salário (04/09)</div>
            ...
            <div style="font-size:0.9rem; font-weight:bold; color:#0F172A; margin-bottom:2px;">📅 Janela Adiantamento (15/09)</div>
            <div style="font-size:1.3rem; font-weight:800; color:#0284C7;">{fmt_brl(entradas_adiantamento_pix)}</div>
            <hr style="margin:8px 0; border:0.5px solid #CBD5E1;">
            <div style="font-size:0.95rem; font-weight:bold; color:#0F172A; margin-bottom:2px;">💰 Total Operacional em Conta</div>
            <div style="font-size:1.5rem; font-weight:800; color:#0F172A;">{fmt_brl(total_receita_conta)}</div>
        </div>
        """, unsafe_allow_html=True)

# --- 5. MAPEAMENTO GERAL DE DÍVIDAS (APENAS TEXTO, SEPARADO EM DOIS BLOCOS) ---
with st.container(border=True):
    st.markdown('<div class="card-header-navy">💳 MAPEAMENTO GERAL DE DÍVIDAS E STATUS DE ACORDOS</div>', unsafe_allow_html=True)

    def calcular_progresso_divida(df_base, tipo_lista="atrasadas"):
        rastreados = []
        ...
        for idx, row in df_base.iterrows():
            ...
            # Nas dívidas no prazo, tudo é considerado "Acordado/No Prazo"
            if tipo_lista == "no_prazo": is_acordado = True
            
            qtd_pagas, total_pago = 0, 0.0
            # CRUZAMENTO RIGOROSO: Só conta se a categoria de saída for Parcelamento/Acordo/Divida
            ...
            cols_num_parc = [c for c in df_base.columns if any(p in c.lower() for p in ['nº', 'num', 'parcelas', 'qtd', 'quantidade'])]
            num_parc = 1
            ...
            if is_acordado and num_parc <= 1:
                num_parc = 36 # Fallback para acordos sem prazo mapeado
    ...
    # BLOCO 1: DÍVIDAS ATRASADAS (DA ABA DIVIDAS)
    with col_div1:
        st.markdown("<h4 style='color:#FF5722; margin-bottom:15px;'>⚠️ Dívidas Atrasadas (Aba Dívidas)</h4>", unsafe_allow_html=True)
        div_atrasadas = calcular_progresso_divida(df_dividas, "atrasadas")
        
        if div_atrasadas:
            for d in div_atrasadas:
                cor_tag = "#0284C7" if d['Is_Acordado'] else "#FF5722"
                bg_tag = "#E0F2FE" if d['Is_Acordado'] else "#FEE2E2"
                status_txt = "Acordo Ativo" if d['Is_Acordado'] else "Pendente"
                pct = min(100.0, (d['Parcelas_Pagas'] / d['Num_Parcelas']) * 100) if d['Num_Parcelas'] > 0 else 0
                
                html_card = f"""
                <div style="background:#F8FAFC; padding:14px; border-radius:8px; border:1px solid #CBD5E1; margin-bottom:10px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                        <span style="font-weight:800; font-size:1.05rem; color:#0F172A;">{d['Credor']}</span>
                        <span style="background:{bg_tag}; color:{cor_tag}; border:1px solid {cor_tag}; padding:2px 8px; border-radius:10px; font-weight:700; font-size:0.75rem;">{status_txt}</span>
                    </div>
                """
                if d['Is_Acordado']:
                    html_card += f"""
                    <div style="font-size:0.85rem; color:#334155; margin-bottom:4px;"><b>Progresso:</b> {d['Parcelas_Pagas']} de {d['Num_Parcelas']} pagas</div>
                    <div style="background-color:#E2E8F0; border-radius:4px; height:6px; width:100%; overflow:hidden; margin-bottom:8px;">
                        <div style="background-color:#0284C7; width:{pct:.1f}%; height:100%;"></div>
                    </div>
                    """
                html_card += f"""
                    <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.9rem; color:#334155;">
                        <span>Original: <b>{fmt_brl(d['Valor_Total'])}</b></span>
                        <span style="color:#FF5722; font-weight:700;">Falta: {fmt_brl(d['Saldo_Restante'])}</span>
                    </div>
                </div>
                """
                st.markdown(html_card, unsafe_allow_html=True)
        else:
            st.info("Nenhuma dívida cadastrada ou processando.")

    # BLOCO 2: DÍVIDAS NO PRAZO (ABA PARCELAMENTOS FIXOS)
    with col_div2:
        st.markdown("<h4 style='color:#10B981; margin-bottom:15px;'>✅ Dívidas no Prazo (Parcelamentos Fixos)</h4>", unsafe_allow_html=True)
        div_prazo = calcular_progresso_divida(df_fixas, "no_prazo")
        
        if div_prazo:
            for d in div_prazo:
                pct = min(100.0, (d['Parcelas_Pagas'] / d['Num_Parcelas']) * 100) if d['Num_Parcelas'] > 0 else 0
                
                # Se for recorrente sem limite de parcelas
                if d['Num_Parcelas'] <= 1:
                    html_prog = f"<div style='font-size:0.85rem; color:#334155; margin-bottom:8px;'><b>Despesa Contínua</b> (Gasto Mês: {fmt_brl(d['Total_Pago'])})</div>"
                else:
                    html_prog = f"""
                    <div style="font-size:0.85rem; color:#334155; margin-bottom:4px;"><b>Progresso:</b> {d['Parcelas_Pagas']} de {d['Num_Parcelas']} pagas</div>
                    <div style="background-color:#E2E8F0; border-radius:4px; height:6px; width:100%; overflow:hidden; margin-bottom:8px;">
                        <div style="background-color:#10B981; width:{pct:.1f}%; height:100%;"></div>
                    </div>
                    """
                ...
                <div style="background:#F8FAFC; padding:14px; border-radius:8px; border:1px solid #CBD5E1; margin-bottom:10px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:6px;">
                        <span style="font-weight:800; font-size:1.05rem; color:#0F172A;">{d['Credor']}</span>
                        <span style="background:#D1FAE5; color:#059669; border:1px solid #10B981; padding:2px 8px; border-radius:10px; font-weight:700; font-size:0.75rem;">Em Dia</span>
                    </div>
                    {html_prog}
                    <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.9rem; color:#334155;">
                        <span>Total/Mês: <b>{fmt_brl(d['Valor_Total'])}</b></span>
                        <span style="color:#0F172A; font-weight:700;">Restante: {fmt_brl(d['Saldo_Restante'])}</span>
                    </div>
                </div>
                """, unsafe_allow_html=True)
        else:
            st.info("Nenhum parcelamento no prazo localizado.")

# --- 6. GRÁFICO EM BARRA POR TIPO DE GASTO ---
with st.container(border=True):
    ...
    if not df_saidas.empty and total_saidas_pix > 0:
        try:
            ...
            df_bar['Label_Text'] = df_bar.apply(lambda r: f"<b>{r['Pct']:.1f}%</b> ({fmt_brl(r['Valor_Clean'])})", axis=1)
            ...
            # Garante que o texto fique sempre fora da barra para leitura clara
