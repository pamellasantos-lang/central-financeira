...
# Função para gerar barras de progresso HTML contínuas (sem quebra de linha)
...
with col1:
    st.markdown(f'<div style="{card_style}"><small style="color: #566573; font-weight: bold;">Entradas (PIX)</small><h2 style="color: #0F2537; margin: 4px 0;">R$ {total_pix:,.2f}</h2><span style="color: #1C2833; font-size: 0.9em;">Dias 05 e 15</span></div>'.replace(',','_').replace('.',',').replace('_','.'), unsafe_allow_html=True)

with col2:
    st.markdown(f'<div style="{card_style}"><small style="color: #566573; font-weight: bold;">Despesas Fixas</small><h2 style="color: #0F2537; margin: 4px 0;">R$ {total_fixas:,.2f}</h2><span style="color: #1C2833; font-size: 0.9em;">Carro (x2) + Mãe</span></div>'.replace(',','_').replace('.',',').replace('_','.'), unsafe_allow_html=True)

with col3:
    st.markdown(f'<div style="{card_style}"><small style="color: #566573; font-weight: bold;">Reserva Essenciais</small><h2 style="color: #0F2537; margin: 4px 0;">R$ {reserva_pix_total:,.2f}</h2><span style="color: #1C2833; font-size: 0.9em;">Valor retido do PIX</span></div>'.replace(',','_').replace('.',',').replace('_','.'), unsafe_allow_html=True)

with col4:
    st.markdown(f'<div style="{card_style}"><small style="color: #566573; font-weight: bold;">Livre p/ Dívidas</small><h2 style="color: #0F2537; margin: 4px 0;">R$ {sobra_total:,.2f}</h2><span style="color: #1C2833; font-size: 0.9em;">Sobra Real no Mês</span></div>'.replace(',','_').replace('.',',').replace('_','.'), unsafe_allow_html=True)
...
with col_q1:
    st.markdown(f'<div style="{card_style}"><h4 style="color: #0F2537; margin-top:0; margin-bottom: 10px;">Ação no Dia 05 (Recebe R$ 2.200)</h4><p style="margin: 4px 0;">1. Pague as Fixas: <b>R$ {fixas_dia5:,.2f}</b></p><p style="margin: 4px 0;">2. Guarde para Essenciais: <b>R$ {reserva_pix_dia5:,.2f}</b></p><hr style="border: 0.5px solid #AED6F1; margin: 8px 0;"><p style="margin: 4px 0; font-size: 1.05em;"><b>Sobra no dia 05:</b> R$ {sobra_dia5:,.2f}</p></div>'.replace(',','_').replace('.',',').replace('_','.'), unsafe_allow_html=True)

with col_q2:
    st.markdown(f'<div style="{card_style}"><h4 style="color: #0F2537; margin-top:0; margin-bottom: 10px;">Ação no Dia 15 (Recebe R$ 1.850)</h4><p style="margin: 4px 0;">1. Pague as Fixas: <b>R$ {fixas_dia15:,.2f}</b></p><p style="margin: 4px 0;">2. Guarde para Essenciais: <b>R$ {reserva_pix_dia15:,.2f}</b></p><hr style="border: 0.5px solid #AED6F1; margin: 8px 0;"><p style="margin: 4px 0; font-size: 1.05em;"><b>Sobra no dia 15:</b> R$ {sobra_dia15:,.2f}</p></div>'.replace(',','_').replace('.',',').replace('_','.'), unsafe_allow_html=True)
...
with col_m1:
    st.markdown(f'<div style="{card_style}"><h4 style="color: #0F2537; margin-top:0; margin-bottom: 10px;">Visão do Flash (VR)</h4><p style="margin: 6px 0;">• <b>Total Recebido:</b> R$ {vr_total:,.2f}</p><p style="margin: 6px 0;">• <b>Repasse Mãe:</b> R$ {vr_mae:,.2f}</p><p style="margin: 6px 0;">• <b>Seu Saldo Livre (Gasolina/Lucca):</b> R$ {vr_disponivel_livre:,.2f}</p></div>'.replace(',','_').replace('.',',').replace('_','.'), unsafe_allow_html=True)
